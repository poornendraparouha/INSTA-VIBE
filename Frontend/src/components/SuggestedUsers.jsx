        ...
        <div className='mt-8 px-4'>
...

            {
                suggestedUsers?.map((user) => {
                    return (
                        <div key={user._id} className="flex items-center justify-between mb-4 hover:bg-gray-50 rounded-lg p-2 transition duration-200"
>
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="leading-tight"><Link to={`/profile/${user?._id}` } className="text-sm font-medium hover:underline">{user?.username}</Link></h1>
